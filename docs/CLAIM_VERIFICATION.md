...
| C02 | Sources and consumers are independent | `src/database.ts`, `src/ui-command.ts` | `test/database.test.mjs`, `test/ui-command.test.mjs` | `pnpm test` | PASS | P0 | Separate tables, operations, and removal paths. |
| C03 | Governance and durable writes require the current direct human event | `src/command-source.ts`, `src/ui-command.ts` | `test/plugin.test.mjs`, `test/ui-command.test.mjs` | `pnpm test:security` | PASS | P0 | Public relationship verbs fail; the plugin exports no model tool. |
...
| C08 | Snapshots contain selected messages and use separated hashed tokens | `src/sharing.ts`, `src/database.ts` | `test/sharing.test.mjs`, `test/security.test.mjs` | `pnpm test:security` | PASS | P0 | Access and append purposes, expiry, revocation, and atomic limits are distinct. |
| C09 | Schema 3 migration preserves data, creates a backup, and tolerates a two-process race | `src/database.ts` | `test/migration.test.mjs`, `test/fixtures/open-store.mjs` | `pnpm test:migration` | PASS | P0 | Complex fixture, rollback, restart, unknown-schema, and process-race paths pass. |
...
| C11 | The plugin uses published DSH interfaces without a core patch | `src/index.ts`, `src/client/index.tsx`, `package.json` | `test/contract.test.mjs`, packaging checks | `pnpm typecheck` and `pnpm package:check` | PASS | P0 | Public imports and published client slots only. |
...
| C14 | Tarball and source installs uninstall cleanly | `package.json`, `cordis.yml` | real DSH profile evidence | exact-version `dsh plugin add/remove` and `--dump-config` | PASS | P1 | Verified locally for rc.6 and rc.7. |
...
| C17 | Hosted Ubuntu, Windows, macOS and Node 22/24 CI passes | `.github/workflows/ci.yml` | GitHub Actions | hosted workflow | UNVERIFIED | P1 | Workflow is authored but the repository has not been uploaded. |
| C18 | npm trusted publication with provenance works | `.github/workflows/release.yml` | npm trusted publisher and tagged GitHub workflow | tagged release | UNVERIFIED | P1 | Configure the npm trusted publisher and pass hosted CI before publishing. |
| C19 | Prompt injection is prevented | `src/serialization.ts`, `src/index.ts` | serialization and injection tests | `pnpm test:security` | PASS | P0 | The README explicitly makes no prevention guarantee and treats memory as untrusted data. |
| C20 | Provenance clearing securely erases physical data | `src/database.ts` | lifecycle tests | `pnpm test` | PASS | P0 | The README explicitly disclaims physical erasure and names retained copies. |

Summary: 16 PASS, 2 PARTIAL, 2 UNVERIFIED, and 0 FAIL. A `PARTIAL` or `UNVERIFIED` row must not be described as complete in release notes.
