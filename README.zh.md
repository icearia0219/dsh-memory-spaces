...
管理器会保留记忆版本及其来源信息，同时把记忆来源和使用会话作为两种独立关系。

![显示有效导入记忆版本的记忆空间管理器](assets/memory-space-memories.png)

使用会话页签显示每个会话的回答时使用方式，但不会因此授予记忆写入能力。

![显示独立使用方式的记忆空间使用会话列表](assets/memory-space-consumers.png)
